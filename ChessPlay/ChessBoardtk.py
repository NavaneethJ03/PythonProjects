...
# Create a function to draw a square on the board
def draw_square(row, col, color):
    # Create a new Canvas object for the square
    square = tk.Canvas(window, width=50, height=50, bg=color)
...
for row in range(8):
    for col in range(8):
        if (row + col) % 2 == 0:
            # Draw light squares
            draw_square(row, col, "#FFF")
        else:
            # Draw dark squares
            draw_square(row, col, "#000")# have to add button functionality 
